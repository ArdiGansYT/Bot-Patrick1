...
const vcard = 'BEGIN:VCARD\n' // OM JANGAN DI UBAH OM
            ...
            + 'FN:RamlanGans\n' // UBAH INI
            + 'ORG:Owner BOT;\n' // UBAH INI
            + 'TEL;type=CELL;type=VOICE;waid=62852829961460:+62 855-5924-0360\n' // UBAH INI
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["62852829961460@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
               ...
               client.sendMessage(from, 'Nih Nomer Owner, Bisnis Sana😅:*',MessageType.text, { quoted: mek} )
                break
				/*case 'loli':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ *PatrickBot* 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓 ]\n\n${body.slice(4)}`})
						}
						reply('𝐃𝐎𝐍𝐄 𝐎𝐌')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ *PatrickBot* 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓 ]\n\n${body.slice(4)}`)
						}
						reply('𝐃𝐎𝐍𝐄 𝐎𝐌')
					}
					break
